...
public class BatchConfiguration {
    ...
    // tag::jobstep[]
    @Bean
    public Job importUserJob(JobBuilderFactory jobs, Step step1, Step step2, Step step3, JobExecutionListener listener) {
        return jobs.get("importUserJob")
                .incrementer(new RunIdIncrementer())
                .listener(listener)
                .flow(step1).on("FAILED").to(step2)
				.from(step1).on("COMPLETED").to(step3)
                .end()
                .build();
    }
